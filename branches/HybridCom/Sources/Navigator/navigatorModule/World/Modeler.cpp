...
TexturePtr Modeler::loadTexture(ModifiedMaterialManager* modifiedMaterialManager, const String& name, const String& group, const TextureExtParamsMap& textureExtParamsMap)
{
    ...
    else
    {
        ...
        if (plugin == "www")
        {
            ...
            naviWWWTexture->setMaxUPS(10);//fps);
            // Awesomium ?
...
// ASA BM added hybridcom plugin
        if (plugin == "hybridcom")
        {
            String hyperlink, object_name;
            int camera , microphone;
            TextureExtParamsMap::const_iterator it;
            it = textureExtParamsMap.find("hyperlink");
            if (it == textureExtParamsMap.end())
                return texture;
            hyperlink = it->second;
            it = textureExtParamsMap.find("camera");
            if (it == textureExtParamsMap.end())
                return texture;
            camera = atoi(it->second.c_str());
            it = textureExtParamsMap.find("microphone");
            if (it == textureExtParamsMap.end())
                return texture;
            microphone = atoi(it->second.c_str());
            it = textureExtParamsMap.find("object_name");
            if (it == textureExtParamsMap.end())
                return texture;
...
// ASA BM
	...
	// ASA addded avatar name to the hyperlink
		Configuration mConfiguration;
		mConfiguration.loadConfig("SolipsisConfiguration.xml");
		std::string mVid = mConfiguration.findParam("MetaverseVid","")->getValueString();
		//hyperlink = hyperlink + "&name=" + object_name + "&vid=" + firstLocalIP;
		hyperlink = hyperlink + "&name=" + object_name + "&vid=" + mVid;
/* LFMODIF
				GUI_MessageBox::getMsgBox()->show("ASA MSG(): hyperlink", hyperlink, 
...
// code commented out 
            NaviLibrary::Navi* naviWWWTexture = NaviLibrary::NaviManager::Get().createNaviMaterial(
                Navigator::getSingletonPtr()->getEntityNaviName(*entity), 
                camera,  // since camera is an int 
                microphone, // since microphone is an int
                false, 70,
                FO_ANISOTROPIC, 
                mtlName);
            naviWWWTexture->loadURL(hyperlink); // hyperlink instead of url

            naviWWWTexture->show(true);
            naviWWWTexture->setMaxUPS(10); // camera instead of fps 
            // Awesomium ?
            //naviWWWTexture->setForceMaxUpdate(fps != 0);
            naviWWWTexture->setOpacity(1.0f);
            // Add 1 listener to follow URL changes
/*
			GUI_MessageBox::getMsgBox()->show("ASA MSG(1)", hyperlink, 
                GUI_MessageBox::MBB_OK, 
                GUI_MessageBox::MBB_INFO);
*/
			Navigator::getSingletonPtr()->addNaviURLUpdatePending(naviWWWTexture->getName(), hyperlink); // hyperlink instad of url
            naviWWWTexture->addEventListener(Navigator::getSingletonPtr());
//
        }
...
void Modeler::releaseTexture(ModifiedMaterialManager* modifiedMaterialManager, const String& name, const TextureExtParamsMap& textureExtParamsMap)
{
    ...
    if (it == textureExtParamsMap.end())
        return;
    String plugin = it->second;
    if (plugin == "www" || plugin == "hybridcom")
    {
...
void Modeler::startEffect(ModifiedMaterialManager* modifiedMaterialManager, const String& name,TextureExtParamsMap &textureExtParamsMap)
{
	...
	else if (plugin == "hybridcom")
    {
        ...
        object_name = it->second;
		    ...
		    // ASA addded avatar name to the hyperlink
			hyperlink = hyperlink + "&name=" + object_name;
        ...
        if (navi == 0)
        {
            ...
            NaviLibrary::Navi* naviWWWTexture = naviMgr.createNaviMaterial(
                Navigator::getSingletonPtr()->getEntityNaviName(*(object->getEntity())), 
                camera, microphone,  // ASA TB changed 
                false, 70,
                FO_ANISOTROPIC, 
                mtlName);
            naviWWWTexture->loadURL(hyperlink);
            naviWWWTexture->show(true);
            //naviWWWTexture->setMaxUPS(fps); // ASA commented out
            // Awesomium ?
            //naviWWWTexture->setForceMaxUpdate(fps != 0);
            naviWWWTexture->setOpacity(1.0f);
            // Add 1 listener to follow URL changes
			/*
				GUI_MessageBox::getMsgBox()->show("ASA MSG ()", hyperlink, 
                GUI_MessageBox::MBB_OK, 
                GUI_MessageBox::MBB_INFO);
			*/
            Navigator::getSingletonPtr()->addNaviURLUpdatePending(naviWWWTexture->getName(), hyperlink);
            naviWWWTexture->addEventListener(Navigator::getSingletonPtr());
        }
        else
        {
            ...
            {
                navi->show(true);
                navi->setMaxUPS(10); //ASA put value to 10
                // Awesomium ?
                //navi->setForceMaxUpdate(fps != 0);
                navi->setOpacity(1.0f);
                // Awesomium ! WebCore update not re-entrant !
