...
void CyberX3D::ParserAddRoute(Node *node)
{
	RouteNode * rNode = (RouteNode*) node;
	//todo cast
	SFString sourceNodeName = rNode->getFromNode();
	SFString sourceNodeTypeName = rNode->getFromField();
	SFString targetNodeName = rNode->getToNode();
	SFString targetNodeTypeName = rNode->getToField();

	GetParserObject()->addRoute(sourceNodeName.getValue(), sourceNodeTypeName.getValue(), targetNodeName.getValue(), targetNodeTypeName.getValue());
}
