...
const Attributes& attributes)
{
    ...
    for (unsigned int index = 0; index < len; index++)
    {
		...
		else 
		{
			if (attrName=="DEF")
			{
				node->setName(XMLCh2Char(attrValue));
			}
			else if (attrName=="USE")
			{
				Node * defNode = GetParserObject()->findNode(XMLCh2Char(attrValue));
				if (defNode)  node->setAsInstanceNode(defNode);
			}
			else
	...
	ParserAddNode(node);
	ParserPushNode(node);
	if(node->isRouteNode()) ParserAddRoute(node);
}
