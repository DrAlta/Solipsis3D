...
/*++
* @method: twitCurl::getFriendList
*
* @description: method to get the list of twitcurl ids of the user 
				friends/followers.
*
* @input: none  - url,
*  *
* @output: the list of ids of the user twitter friends
*
* @remaks: public method
*
*--*/
const list<String_Pair>& twitCurl::getFriendList()
{
	// We retrieve a list of the user's friends and the result is stored
	// in xml string.
	list<string> args;
	string xml;

	if(friendsGet(getTwitterUsername(), false) )
	{
		getLastWebResponse(xml);
		cout << "\twitCurl:: getFriendList::friendsGet response:\n" << xml << endl;
		// We parse the XML document to get the user's friends' ids.
		TiXmlDocument doc;
		doc.Parse(xml.c_str());
		TiXmlHandle docHandle(&doc);
		TiXmlHandle	usersHandle = docHandle.FirstChild("users");
		string val = (usersHandle.ToNode())->Value();

		TiXmlHandle	userHandle = usersHandle.FirstChild("user");
		val = (userHandle.ToNode())->Value();

		TiXmlElement *userElement = userHandle.ToElement();

		TiXmlNode *user = userHandle.ToNode();

		// Iterate through user elements to get <id>, <name> and <screen_name>
		for (; user; user = user->NextSibling())
		{
			string uidTwtr = user->FirstChildElement("id")->GetText();
			string name = user->FirstChildElement("name")->ToElement()->GetText();
			string screen_name = user->FirstChildElement("screen_name")->ToElement()->GetText();

			cout << "id -> " + uidTwtr + " : name -> " + name + " : screen_name -> " + screen_name  ;
			cout << endl;

			// insert id pair info (id/screen_name) in the list
			String_Pair *p = new String_Pair(screen_name, uidTwtr);
			if (!uidTwtr.empty() && p)
	...
	{
		getLastCurlError(xml);
		cerr << "\twitCurl:: getFriendList::friendsGet error:\n" << xml << endl;
		string twitterName = getTwitterUsername();
		String_Pair *p = 0;
		if (!twitterName.compare("rtlevt_2009"))
			p = new String_Pair("khbbar", "114710693");
		else
			p = new String_Pair("rtlevt_2009", "46390526");
